...
| Area | Impact |
|---|---|
| BottomSheet public resource migration | Adds the `BottomSheetActions` public scene opener layer and moves scattered funding, security, account, Discover, and Auth sheet bodies to governed public business components. No product route, business rule, copy semantic, icon semantic, or risk-level behavior changes. Affects BottomSheet callers on funding, security log, account balance/details, portfolio/trade, Discover profile, and Auth phone/language/error flows. |
| BottomSheet QA boundary | `pnpm qa:components` now blocks direct page/non-public `bottomSheet.show/push/update`, direct `bottomSheetPresets`, and redeclaration of governed sheet body names outside `src/components/business`. |
