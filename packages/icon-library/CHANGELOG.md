...
- Recorded the icon-library patch version for the current neutral primary icon default governance.
- No icon glyphs, names, or runtime mappings were changed by this record.
...
- Added the package-local `local/types.ts` bridge required by vendored local icon assets.
- Confirmed bottom-navigation account usage should consume existing governed `icon.wallet.balance` for asset semantics.

## 4.1.1 - 2026-05-26

- Established the independent `@dpf/icon-library` package boundary.
- Migrated icon registry, schema, naming, usage, QA rules, runtime registry, local icon component map, and local Iconsax assets into the package.
- Kept `src/icons` and `src/components/AppIcon.tsx` as compatibility runtime paths for the first migration stage.
